<?php
	...
	$GLOBALS['tables']['users'] = ['_id_'=>'INTEGER AUTOINCREMENT','userMail'=>'TEXT NOT NULL UNIQUE','userPass'=>'TEXT NOT NULL','userWord'=>'TEXT NOT NULL',
	 ...
	 */
	function users_create($data = [],$params = []){
		$valid = ['userName'=>0,'userBirth'=>0,'userMail'=>0,'userPass'=>0,'userGender'=>0,'userNick'=>0];
		...
		if(!isset($params['db.file'])){$params['db.file'] = $GLOBALS['api']['users']['db'];}

		$data['userName'] = preg_replace('/[^a-zA-ZáéíóúÁÉÍÓÚ ,]*/','',$data['userName']);
		if(empty($data['userName'])){return ['errorDescription'=>'NAME_ERROR','file'=>__FILE__,'line'=>__LINE__];}
		if(!preg_match($GLOBALS['api']['users']['reg.mail.match'],$data['userMail'])){return ['errorDescription'=>'EMAIL_ERROR','file'=>__FILE__,'line'=>__LINE__];}
		/* Check duplicate mail */
		if(users_getSingle('(userMail = \''.$data['userMail'].'\')',$params)){return ['errorDescription'=>'EMAIL_DUPLICATED','file'=>__FILE__,'line'=>__LINE__];}
		...
		$data = array_merge($data,['userWord'=>$magicWordPre,'userRegistered'=>$date,'userStatus'=>0,'userModes'=>',regular,','userCode'=>$userCode]);

		$r = sqlite3_insertIntoTable2($GLOBALS['api']['users']['table'],$data,$params,'users');
		if(isset($r['errorDescription'])){return array_merge($r,['file'=>__FILE__,'line'=>__LINE__]);}
		if( !($userOB = users_getSingle('(id = \''.$r['id'].'\')',$params)) ){return ['errorDescription'=>'UNKNOWN_ERROR','file'=>__FILE__,'line'=>__LINE__];}
		$userOB = array_merge($userOB,['userCode'=>$userCode]);
		return $userOB;
	}
	function users_update($id = 0,$data = [],$params = []){
		/* In this case, must be two different functions instead of a 'save' method
		 * just because 'update' can set fields 'create' cant */
		include_once('inc.strings.php');
...
//FIXME revisar
		...
		$userOB = users_getSingle('(userMail = \''.$db->escapeString($userMail).'\' AND userPass = \''.$db->escapeString($userPass).'\')',array('db'=>$db));
		if(!$userOB){if($shouldClose){sqlite3_close($db);}return array('errorDescription'=>'WRONG_USER_OR_PASS','file'=>__FILE__,'line'=>__LINE__);}
		/* Puede que el usuario no esté confirmado, en dicho caso no se permite loguear */
		if(!isset($userOB['userStatus']) || empty($userOB['userStatus'])){if($shouldClose){sqlite3_close($db);}return array('errorCode'=>1,'errorDescription'=>'USER_NOT_ACTIVE','file'=>__FILE__,'line'=>__LINE__);}
